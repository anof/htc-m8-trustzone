/* emmcpwr10 - power-cycle the eMMC by *lowering its rail voltage*, not by
...
#define CUT_MS 150u		/* VCC held off */
...
int init_module(void)
{
	...
	p_off = (int (*)(void *))kallsyms_lookup_name("regulator_force_disable");
	p_on = (int (*)(void *))kallsyms_lookup_name("regulator_enable");
	printk("emmcpwr10: susp=%p reinit=%p poff=%p pon=%p isen=%p rfoff=%p "
	...
	if (!g_host) {
		printk("emmcpwr10: no mmc0 child\n");
		return -22;
	}
	printk("emmcpwr10: class_dev=%p host=%p parent=%p index=%d\n", g_dev,
	       g_host, *(void **)g_host, *(int *)((char *)g_host + 0x198));
	if (*(void **)g_host != pdev) {
		printk("emmcpwr10: parent mismatch - aborting\n");
		return -26;
	}
	reg = regulator_get(NULL, "8941_l20");
	if (!reg || (unsigned long)reg >= (unsigned long)-4095) {
		printk("emmcpwr10: regulator_get failed (%p)\n", reg);
		return -23;
	}
	printk("emmcpwr10: regulator=%p enabled=%d\n", reg, p_isen(reg));

	rc = p_susp(g_host);
	printk("emmcpwr10: suspend -> %d\n", rc);
	suspend_ok = (rc == 0);

	if (suspend_ok) {
		rc = p_off(reg);
		e = p_isen(reg);
		printk("emmcpwr10: force_disable -> rc=%d, enabled=%d\n", rc, e);
		if (e == 0) {
			cut = 1;	/* VCC is really down: card POR, WP gone */
			msleep(CUT_MS);
		} else {
			p_on(reg);	/* put it back, nothing was achieved */
			return -40;
		}
		rc = p_on(reg);
		e = p_isen(reg);
		printk("emmcpwr10: enable -> rc=%d, enabled=%d\n", rc, e);
		for (i = 0; !e && i < 5; i++) {
			msleep(100);
			p_on(reg);
			e = p_isen(reg);
			printk("emmcpwr10: re-enable try %d -> enabled=%d\n",
			       i + 1, e);
		}
		if (!e) {
			printk("emmcpwr10: RAIL DID NOT COME BACK\n");
			return -41;
		}
		msleep(200);
	}
	...
	p_power_off(g_host);
	msleep(100);
	printk("emmcpwr10: after power_off: enabled=%d\n", p_isen(reg));
	p_power_up(g_host);
	e = p_isen(reg);
	printk("emmcpwr10: after power_up: enabled=%d\n", e);
	if (!e) {
		p_on(reg);
		msleep(100);
		printk("emmcpwr10: re-enabled after power_up: enabled=%d\n",
		       p_isen(reg));
	}
	rc = p_reinit(g_host);
	printk("emmcpwr10: mmc_reinit -> %d\n", rc);
	if (rc && suspend_ok) {
		msleep(300);
		p_power_off(g_host);
		msleep(100);
		p_power_up(g_host);
		rc = p_reinit(g_host);
		printk("emmcpwr10: mmc_reinit retry -> %d\n", rc);
	}
	if (rc)
		return -25;

	printk("emmcpwr10: DONE cut=%d, card back, rail enabled=%d\n", cut,
	       p_isen(reg));
	if (!cut)
		return -40;
	return 0;
